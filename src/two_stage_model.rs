...
pub struct TwoStageClassifier {
    stage1: Option<RandomForest>,
    stage2: Option<RandomForest>,
...
impl TwoStageClassifier {
    ...
    pub fn train(&mut self, train_data: &ProcessedDataset) -> Result<()> {
        ...
        let stage1_labels: Array1<u8> = train_data
            .labels()
            .iter()
        ...
        // Stage 2 - Binary classification on NON_CONFIRMED only
        ...
        info!(
            "Stage 2 training on {} NOT_CONFIRMED samples...",
            stage2_indices.len()
        );
        ...
        for (new_idx, &old_idx) in stage2_indices.iter().enumerate() {
            ...
            stage2_labels[new_idx] = if train_data.labels()[old_idx] == 1 {
                0
            } else {
                1
            };
        ...
        let mut feature_importance: Vec<(String, f64)> = combined_importance
            .iter()
    ...
    pub fn predict_proba(&self, features: &Array2<f64>) -> Result<Array2<f64>> {
        ...
        let stage1_probs = s1.predict_proba(features);
        ...
        if !stage2_indices.is_empty() {
            ...
            let stage2_probs = s2.predict_proba(&stage2_features);
            ...
            for (new_idx, &old_idx) in stage2_indices.iter().enumerate() {
                ...
                probs[[old_idx, 1]] = p_not_confirmed * p_candidate;
                probs[[old_idx, 2]] = p_not_confirmed * p_false_pos;
            }
        }
